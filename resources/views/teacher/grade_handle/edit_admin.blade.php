...
<div class="min-w-full flex items-center justify-center p-6" style="min-height: 560px">
    <form action="{{ route('admin.update.admin', $admin->id) }}" method="post" class="w-full max-w-3xl bg-white rounded-lg p-8 shadow">
        @csrf
        @method('PUT')
        <div class="w-full">
        ...
        <div class="block md:flex align-center justify-between my-2 gap-5">
            ...
            <div class="md:w-1/2 w-full">
                ...
                <input type="tel" id="phone_number" name="phone_number" class="form-input w-full rounded border-gray-300 @error('phone_number') border-red-500 @enderror" value="{{ old('phone_number') ?? $admin->phone_number }}">
                @error('phone_number')
                <span class="text-red-500 text-sm">{{ $message }}</span>
                @enderror
            </div>
